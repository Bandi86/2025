      ...
      if (existing) {
        ...
        try {
          ...
          if (authorId && authorId !== userId) {
            const username = (req.user as any)?.username || 'Someone';
            const targetTitle = postId
              ? (await prisma.post.findUnique({ where: { id: postId }, select: { title: true } }))
                  ?.title
              : 'your comment';
            ...
            await prisma.notification.create({
              data: {
                type: 'LIKE',
                message: `${username} liked ${targetTitle}`,
                userId: authorId,
                actorId: userId,
