...
// Use CommonJS version of node-fetch
import pkg from 'node-fetch';
const { default: fetch } = pkg;
import colors from 'colors/safe';env node
...
const TEST_ENDPOINTS = [
  ...
  { path: '/api/test/validation-error', method: 'POST', name: 'Validation Error',
    body: { name: 'Jo', age: 15 } },
  ...
  { path: '/api/test/forbidden-error?resource=post&action=delete', method: 'GET', name: 'Forbidden Error' },
  { path: '/api/test/conflict-error?email=test@example.com', method: 'GET', name: 'Conflict Error' },
  ...
  { path: '/api/test/custom-error?status=418&code=TEAPOT&severity=LOW', method: 'GET', name: 'Custom API Error' },
