...
import sqlite3
import re
import json        # Pontos egyezés keresése
        ...
        for team_id, name, norm_name in teams:
            if (normalized_name.lower() in name.lower() or
                ...
                conn.close()
                return team_idrt datetime, timedelta
...
class ResultUpdater:
    ...
    def find_team_id(self, team_name: str) -> Optional[int]:
        ...
        cursor.execute("SELECT id FROM teams WHERE name = ? OR normalized_name = ?",
                      (team_name, normalized_name))
        result = cursor.fetchone()
        ...
        # Fuzzy keresés - hasonló nevű csapatok
    ...
    def process_pdf_for_results(self, pdf_path: str, pdf_date: str = None):
        """Feldolgoz egy PDF fájlt eredmények kinyeréséhez"""
        import subprocess
        ...
        try:
            ...
            text = result.stdout
            extracted_results = self.extract_results_from_pdf_text(text, pdf_date)

            if not extracted_results:
                self.logger.info(f"Nem találtunk eredményeket: {pdf_path}")
                return

            # Meccsek azonosítása és frissítése
            matches_to_update = self.find_matches_to_update()
            updated_count = 0

            for pdf_result in extracted_results:
                home_team_id = self.find_team_id(pdf_result['home_team'])
                away_team_id = self.find_team_id(pdf_result['away_team'])

                if not home_team_id or not away_team_id:
                    self.logger.warning(f"Nem található csapat: {pdf_result['home_team']} vs {pdf_result['away_team']}")
                    continue

                # Megfelelő meccs keresése
                for match in matches_to_update:
                    if (match['home_team_id'] == home_team_id and
                        match['away_team_id'] == away_team_id):

                        if self.update_match_result(match['id'], pdf_result['home_score'], pdf_result['away_score'], 0.85):
                            updated_count += 1
                        break

            self.logger.info(f"Frissítve {updated_count} meccs eredménye a {pdf_path} alapján")

        except Exception as e:
            self.logger.error(f"Hiba a PDF feldolgozás során {pdf_path}: {e}")

    def batch_update_from_directory(self, pdf_directory: str):
        """Kötegelt frissítés egy könyvtárból"""
        ...
        self.logger.info(f"Találtunk {len(pdf_files)} PDF fájlt a frissítéshez")

        for pdf_file in pdf_files:
            self.logger.info(f"Feldolgozás: {pdf_file.name}")
            self.process_pdf_for_results(str(pdf_file))

    def update_from_api_data(self, api_results: List[Dict]):
        """Frissítés API adatokból"""
        updated_count = 0

        for result_data in api_results:
            home_team_id = self.find_team_id(result_data['home_team'])
            away_team_id = self.find_team_id(result_data['away_team'])

            if not home_team_id or not away_team_id:
                continue

            # Meccs keresése dátum alapján
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            """, (home_team_id, away_team_id, result_data['date']))

            match = cursor.fetchone()
            conn.close()

            if match:
                home_score, away_score = map(int, result_data['result'].split('-'))
                if self.update_match_result(match[0], home_score, away_score, 0.95):
                    updated_count += 1

        self.logger.info(f"API-ból frissítve {updated_count} meccs eredménye")
    ...
    def generate_update_report(self) -> Dict:
        ...
        cursor.execute("""
            SELECT h.match_id, h.date, ht.name, at.name, h.home_score, h.away_score, h.updated_at
            FROM historical_matches h
            LEFT JOIN teams ht ON h.home_team_id = ht.id
            LEFT JOIN teams at ON h.away_team_id = at.id
