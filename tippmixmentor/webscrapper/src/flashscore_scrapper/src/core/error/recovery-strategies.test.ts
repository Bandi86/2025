...
describe('ErrorRecoveryManager', () => {
  ...
  describe('executeRecovery', () => {
    ...
    it('should handle recovery action failures gracefully', async () => {
      ...
      actions[0].execute = jest.fn().mockRejectedValue(new Error('Recovery failed'));
  ...
  describe('createPartialDataRecovery', () => {
    ...
    it('should throw error when required fields are missing', async () => {
      ...
      await expect(recoveryAction.execute())
        .rejects.toThrow('Insufficient data for partial recovery');
    });
  });
});
...
describe('GracefulDegradation', () => {
  describe('executeWithFallback', () => {
    it('should return primary operation result when successful', async () => {
      const primaryOp = jest.fn().mockResolvedValue({ id: '1', name: 'Primary', data: 'complete' });
      const fallbackOp = jest.fn().mockResolvedValue({ id: '1', name: 'Fallback' });
    ...
    it('should throw original error when fallback lacks required fields', async () => {
      ...
      const primaryOp = jest.fn().mockRejectedValue(primaryError);
      const fallbackOp = jest.fn().mockResolvedValue({ id: '1' }); // Missing 'name'
  ...
  describe('collectPartialResults', () => {
    it('should collect successful results and track failures', async () => {
      const operations = [
        ...
        jest.fn().mockRejectedValue(new Error('op2 failed')),
        jest.fn().mockResolvedValue('result3'),
        jest.fn().mockRejectedValue(new Error('op4 failed'))
      ];
    ...
    it('should throw error when minimum success count not met', async () => {
      const operations = [
        jest.fn().mockRejectedValue(new Error('op1 failed')),
        jest.fn().mockRejectedValue(new Error('op2 failed'))
      ];
  ...
  describe('createResilientCollector', () => {
    it('should collect data from all successful extractors', async () => {
      const extractors = {
        id: jest.fn().mockResolvedValue('123'),
        name: jest.fn().mockResolvedValue('Test Name'),
        optional: jest.fn().mockResolvedValue('Optional Data')
      };
    ...
    it('should continue with partial data when optional extractors fail', async () => {
      const extractors = {
        id: jest.fn().mockResolvedValue('123'),
        name: jest.fn().mockResolvedValue('Test Name'),
        optional: jest.fn().mockRejectedValue(new Error('Optional failed'))
      };
    ...
    it('should throw error when required fields are missing', async () => {
      const extractors = {
        id: jest.fn().mockResolvedValue('123'),
        name: jest.fn().mockRejectedValue(new Error('Name extraction failed')),
        optional: jest.fn().mockResolvedValue('Optional Data')
      };
